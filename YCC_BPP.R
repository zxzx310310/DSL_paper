...
for (loop in 1:10) {
  ...
  initial_pop <- function(good_data, require_goods, non_require_goods, non_require_values) {
    ...
    user_value <- length(require_goods)+length(non_require_goods)
    temp_good <- good_data #先將原始資料暫時給另外一個變數使用
    get_index <- sample(which(temp_good$'Selected'!=1), user_value) #隨機抓出Selected欄位不等於1的列
    temp_good$'Selected'[get_index] <- 1 #將被選擇的欄位改為1 
    selected_good <- temp_good[temp_good$'Selected'==1,] #將被選擇的商品放入新變數中
    selected_good <- selected_good[sample(nrow(selected_good), user_value), ]
    selected_good$'種類' <- factor(selected_good$'種類')
  ...
  #選擇(隨機法):
  #從父母中隨機挑選出兩個染色體, 複製至交配池中
  selection_first <- function(gene_list, pop_amount) {
    #gene_list: 被選擇出的基因清單
    #pop_amount: 人口數量
    gene_list <- gene_list[order(sapply(gene_list, function(x) x$totalFit), decreasing=FALSE)] #遞增排序
    random_pop <- sample(gene_list, pop_amount, replace = TRUE) #隨機挑選符合族群數量的染色體
    return(random_pop)
  }
  
  #交配(雙點交配)-
  cross_over <- function(good_data, gene_list, require_goods, non_require_values, cross_rate) {
