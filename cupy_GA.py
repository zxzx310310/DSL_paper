...
def total_weight(gene_list):
    #gene_list: 被選擇出的基因清單
    for i in range(len(gene_list)):
        sum_weight = cp.sum(gene_list[i]['data.frame'][:,10])
...
def fitness_preference(gene_list, require_goods, non_require_values, preference_table):
    ...
    for i in range(len(gene_list)):
        ...
        gene_list[i]['fitPreference'] = cp.array([reuslt])
        sum_preferenced = (gene_list[i]['data.frame'][:,12]).sum()
        gene_list[i]['totalPreference'] = cp.array([sum_preferenced])
    return(gene_list)
...
def fitness_volume(gene_list, bin_volume):
    ...
    #bin_volume: 箱子的乘積
    for i in range(len(gene_list)):
        sum_volume = cp.sum(gene_list[i]['data.frame'][:,3]) #將最大限制體積減去每個基因的總體積
        ...
        gene_list[i]['fitVolume'] = cp.array([reuslt])
    return(gene_list)
...
def fitness_price(gene_list, limit_price):
    ...
    #limit_price: 價格最高限制
    for i in range(len(gene_list)):
        sum_price = cp.sum(gene_list[i]['data.frame'][:,2]) #將最大限制金額減去每個基因的總金額
        ...
        gene_list[i]['fitPrice'] = cp.array([reuslt])
        gene_list[i]['totalPrice'] = cp.array([sum_price])
    return(gene_list)
...
def fitness_total(gene_list):
    ...
    for i in range(len(gene_list)):
        ...
        gene_list[i]['totalFit'] = cp.array([sum_fit], dtype=cp.float64)
    return(gene_list)
...
def mutation_FN(good_data, gene_list, mutation_rate):
    ...
    for i in range(len(gene_list)):
        ...
        if rnd_mutation_rate <= mutation_rate:
            ...
            gene_list[i]['totalWeight'] = cp.sum(crossAfter[i]['data.frame'][:,10]) #重新計算總重量
