    ...
    for (i in 1:length(gene_list)) {
      ...
      if (sum_volume >=(bin_volume*0.7) & sum_volume <=bin_volume) {
        if (subtraction_volume==0) {
          reuslt <- reuslt + 1*1 #若適應度等於0就給予懲罰值1, e.g. (49795.2-27749.25)/49795.2=0.4427324, 愈接近0表示價格差距越小
        } else {
          reuslt <- reuslt + 2*10 #若適應度大於0就給予懲罰值2
        } 
      } else {
        reuslt <- reuslt + 3*100 #剩下結果將給予懲罰值3
      }
...
tempDF <- data.frame(totFit = totalFitness, totPric = price, totPref = preference, VR = volumeRate)
